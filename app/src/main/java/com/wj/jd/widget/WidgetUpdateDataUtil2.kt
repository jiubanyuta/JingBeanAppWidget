...
object WidgetUpdateDataUtil2 {
    ...
    fun updateWidget(key: String) {
        ...
        page = 1
        UserBean2.todayBean = 0
        UserBean2.ago1Bean = 0
            ...
            override fun onSuccess(result: String) {
                try {
                    ...
                    if (DeviceUtil.getAppVersionName().equals(versionBean.release)) {
                        UserBean2.updateTips = ""
                    } else {
                        UserBean2.updateTips = versionBean.widgetTip
                    }
                } catch (e: Exception) {
                    e.printStackTrace()
                }
        ...
        HttpUtil.getRedPack(thisKey, "https://m.jingxi.com/user/info/QueryUserRedEnvelopesV2?type=1&orgFlag=JD_PinGou_New&page=1&cashRedType=1&redBalanceFlag=1&channel=1&_=" + System.currentTimeMillis() + "&sceneval=2&g_login_type=1&g_ty=ls", object : StringCallBack {
            override fun onSuccess(result: String) {
                try {
                    ...
                    UserBean2.hb = redPacket.data.balance
                    UserBean2.gqhb = redPacket.data.expiredBalance
                    UserBean2.countdownTime = redPacket.data.countdownTime / 60 / 60
                    setData()
                } catch (e: Exception) {
                    e.printStackTrace()
                }
        ...
        HttpUtil.getUserInfo1(thisKey, object : StringCallBack {
            override fun onSuccess(result: String) {
                try {
                    ...
                    UserBean2.jxiang = job.optJSONObject("user").optString("uclass").replace("京享值", "")
                    setData()
                } catch (e: Exception) {
                    e.printStackTrace()
                }
        ...
        HttpUtil.getUserInfo(thisKey, object : StringCallBack {
            override fun onSuccess(result: String) {
                try {
                    val job = JSONObject(result)
                    try {
                        UserBean2.nickName = job.optJSONObject("data").optJSONObject("userInfo").optJSONObject("baseInfo").optString("nickname")
                        UserBean2.userLevel = job.optJSONObject("data").optJSONObject("userInfo").optJSONObject("baseInfo").optString("userLevel")
                        UserBean2.levelName = job.optJSONObject("data").optJSONObject("userInfo").optJSONObject("baseInfo").optString("levelName")
                        UserBean2.headImageUrl = job.optJSONObject("data").optJSONObject("userInfo").optJSONObject("baseInfo").optString("headImageUrl")
                        UserBean2.isPlusVip = job.optJSONObject("data").optJSONObject("userInfo").optString("isPlusVip")
                    } catch (e: Exception) {
                        e.printStackTrace()
                    }
                    try {
                        UserBean2.beanNum = job.optJSONObject("data").optJSONObject("assetInfo").optString("beanNum")
                    } catch (e: Exception) {
                        e.printStackTrace()
                    }
                    setData()
                ...
                try {
                    ...
                    for (i in dataList.indices) {
                        ...
                        if (beanDay > todayTime) {
                            if (detail.amount > 0) {
                                UserBean2.todayBean = UserBean2.todayBean + detail.amount
                            }
                        } else {
                            isFinish = false
                            break
                ...
                try {
                    ...
                    for (i in dataList.indices) {
                        ...
                        if (beanDay < todayTime && beanDay > yesterdayTime) {
                            if (detail.amount > 0) {
                                UserBean2.ago1Bean = UserBean2.ago1Bean + detail.amount
                            }
    ...
    private fun setData() {
        ...
        remoteViews.setTextViewText(R.id.todayBean, "+" + UserBean2.todayBean)
        remoteViews.setTextViewText(R.id.todayBeanNum, UserBean2.todayBean.toString())
        remoteViews.setTextViewText(R.id.oneAgoBeanNum, UserBean2.ago1Bean.toString())
        remoteViews.setTextViewText(R.id.updateTime, "数据更新于:" + getCurrentData())
        remoteViews.setTextViewText(R.id.hongbao, UserBean2.hb)
        try {
            if (getCurrentHH() + UserBean2.countdownTime > 24) {
                remoteViews.setTextViewText(R.id.guoquHb, "明日过期:" + UserBean2.gqhb)
            } else {
                remoteViews.setTextViewText(R.id.guoquHb, "今日过期:" + UserBean2.gqhb)
            }
        } catch (e: Exception) {
            e.printStackTrace()
            remoteViews.setTextViewText(R.id.guoquHb, "今日过期:" + UserBean2.gqhb)
        }
        remoteViews.setTextViewText(R.id.jingXiang, UserBean2.jxiang)

        val cleatIntent = Intent()
        cleatIntent.flags = Intent.FLAG_ACTIVITY_NEW_TASK
        cleatIntent.action = "com.scott.sayhi2"
        ...
        if (TextUtils.isEmpty(UserBean2.headImageUrl)) {
